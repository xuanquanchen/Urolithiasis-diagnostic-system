  ...
  <div>
    <el-input
      ...
      <div style="display: flex;">
        <el-button
        class="button tourist-button"
        @click="tourLogin">游客登录</el-button>
        <el-button
        :loading="logining"
        class="button"
...
export default {
  ...
  methods: {
    tourLogin () {
      this.$emit('tourLogin', this.userName, this.password, 0)
    },
    enterCheck (index) {
    ...
    login () {
      this.$emit('login', this.userName, this.password, 0)
    }
  }
}
</script>
...
.button:hover{
  background-image: linear-gradient(170deg, #446dff 0%, rgba(99, 125, 255, 0.75) 100%);
  color: #fff;
}
.tourist-button {
